...
}, [testimonials.length]);  //testimonials.length as dependency is more robust

return (
<div className="min-h-screen">
{/* Banner Section */}
<section className="py-20 px-4 md:px-8 lg:px-16 mt-16">
...
/>
      ...
      >
        {/* Combine static text with TypeAnimation */}
        <h1 className="text-4xl font-bold">
            Hello, I am Hriday Sehgal.
        </h1>
        <h2 className="text-2xl text-primary">
         I am a&nbsp; {/* Add  non-breaking space */}
          <TypeAnimation
            sequence={[
              'Software Developer',
              2000,
        ...
        <p className="text-lg text-muted-foreground">
          A passionate technologist with knowledge in full-stack development and product management.
          Currently pursuing B.Tech in Computer Science Engineering with a specialization in Health Informatics.
        </p>
        <div className="flex gap-4">
          {/* Resume Button - Google Drive Link */}
          <Button asChild  className="flex items-center gap-2">
            <a href="https://drive.google.com/file/d/1ZzSDnKf3DNbndhiFDt3ocjrOnQLybDXh/view?usp=sharing" target="_blank" rel="noopener noreferrer">
                <FiDownload className="h-5 w-5" /> Resume
            </a>
          </Button>
          {/* Contact Me Button - Linked to Contact Section */}
          <Button
            variant="outline"
            className="flex items-center gap-2"
            onClick={scrollToContact}
            >
              Contact Me <FiArrowRight className="h-5 w-5" />
          </Button>
        </div>
      </motion.div>
    </div>
   ...
   {/* Skills Section */}
   <section id="skills" className="py-16 px-4 md:px-8 lg:px-16 bg-secondary/20">
    <motion.div
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <Tabs defaultValue="technical" className="w-full max-w-4xl mx-auto">
        <TabsList className="grid grid-cols-2 gap-x-4">
          ...
          <TabsTrigger value="management">Management & Soft Skills</TabsTrigger>
        </TabsList>
        <TabsContent value="technical">
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }} // Add whileInView for scroll effect
        ...
        <motion.div
          ...
          >
            {skills.management.map((skill) => (
               <div key={skill.name} className="flex items-center gap-2 p-2 rounded-md hover:bg-black/5 transition-colors">
                {skill.icon} <span className="text-sm">{skill.name}</span>
              </div>
            ))}
          </motion.div>
        </TabsContent>
